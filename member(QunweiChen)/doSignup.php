...
if(empty($name)||empty($email)||empty($password)||empty($repassword)||empty($phone)||empty($national_id)||empty($born_date)||empty($invoice)){
    ...
    header("location: menber_signup.php");
    exit;
}
...
$sqlUser="INSERT INTO menber_list (name, email, password, phone, national_id, born_date, invoice, created_at, valid)
...
if($conn->query($sqlUser) === TRUE){
    ...
    header("location: menber_login.php?messageSuccess=" . urldecode($resultMessage));
    exit;
}else{
